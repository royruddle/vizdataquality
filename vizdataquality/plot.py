...
def _draw_fig(filename, overwrite, **kwargs):
    """
    Internal function, which is called to draw a plot to the screen or save it
    in a file.
    ...
    **kwargs : dictionary
    ...
    if filename is not None:
        if overwrite or not os.path.isfile(filename):
            ...
            print('** WARNING ** vizdataquality, plot.py, _draw_fig(): ' +
                  'Figure not output because a file with the supplied name ' +
                  'already exists.')
            print(filename)
...
# Utility functions
# =============================================================================
def apply_perceptual_discontinuity_individually(input_data,
                                                perceptual_threshold,
                                                axis_limits=None):
    """
    Apply perceptual discontinuity threshold.
    If axis_limits is None then values in range 0 < x < perceptual_threshold
    are set equal to perceptual_threshold.
    Otherwise values in the range 0 < x < max are adjusted so each is
    distinguishable from 0 and max.
    ...
    perceptual_threshold : float
        An absolute value (axis_limits = None) or a percentage (0.0 - 1.0) of
        the axis limit max
    ...
    -------
    Series or DataFrame
        A series (the adjusted values) or dataframe (the adjusted values and,
        if a series was input then an extra column for each group of values
        for plotting as a stacked bar chart).

    """
    # data = input_data.copy()
    # Distinguish between original values and values adjusted by perceptual
    # discontinuity
    ...
    if axis_limits is None or not isinstance(axis_limits, tuple):
        ...
        if type(input_data) is pd.Series:
            # data = input_data.apply(lambda x: x if x <= 0 else max(x,
            # perceptual_threshold)).to_frame()
            # .reset_index().rename(columns={data.name: 'Value',
            # 'count': 'Count'})
            stack = []
            for l1 in range(2):
        ...
        if type(input_data) is pd.Series:
            ...
            def adjust_values(x, max_value, perceptual_threshold):
                if x <= 0 or x >= max_value:
                    return x
                if x < max_value / 2.0:
                    return max(x, perceptual_threshold * max_value)
                else:
                    return min(x, (1.0 - perceptual_threshold) * max_value)

            # Usage within your data transformation
            data = input_data.apply(adjust_values,
                                    args=(max_value, perceptual_threshold))
...
def apply_perceptual_discontinuity_to_group(input_data, perceptual_threshold):
    """
    Apply perceptual discontinuity threshold to a group of values. The values
    are adjusted so each is >= perceptual_threshold * sum, but the sum is
    unchanged.
    That is achieved by increasing values that are below the threshold and
    decreasing values that are above the threshold.
    ...
    -------
    Series or DataFrame
        A series or dataframe, with perceptual discontinuity applied to groups
        of values (e.g. in a stacked bar chart)
    ...
    if isinstance(data, pd.Series):
        ...
        if data.sum() > total:
            ...
            # Reduce the values proportionately, making sure every value is
            # still >= threshold
        ...
        for col in data.columns:
            ...
            data[col] = data[col].apply(
                lambda x: 0.0 if x <= 0.0 else max(x, p))

            if data[col].sum() > total:
                # The sum of the values is > max_value, so calculate the excess
                e = float(data[col].sum() - total)
                sump = float(data[col].sum() - p * (data.shape[1]))
                # Reduce the values proportionately, making sure every value
                # is still >= threshold
                data[col] = data[col].apply(
                    lambda x: x if x <= p else x - e*(x-p)/sump)
...
# =============================================================================
def plotgrid(tasktype, data, num_rows=None, num_cols=None, vert=True,
             xlabels_rotate=0.0, perceptual_threshold=0.05, legend=True,
             components='raw data', gap_threshold=None, show_gaps=True,
             datalabels=False, continuous_value_axis=True, filename=None,
             overwrite=False, fig_kw={}, ax_kw={}, legend_kw={}, **kwargs):
    """
    ...
    legend: boolean
        True (add a legend, if a stacked bar chart is plotted) or False
        (no legend)
    components : list or string
        Only used if tasktype is 'datetime distribution': Component(s) to plot
        ('year', 'month', 'dayofweek', 'hour', 'minute' or 'second'; case
        independent) or 'raw data' (default)
    gap_threshold: None, int or datetime
        Only used if tasktype is 'datetime distribution': None (threshold will
        be based on the component of the data; the default) or value
        (threshold to use). Only used if component is specified.
    show_gaps: boolean
        Only used if tasktype is 'datetime distribution': True (the default)
        or False (draw lines across gaps). Only used if component is specified.
    datalabels : boolean
        Label each data point (False (default) or True)
    continuous_value_axis : boolean
        Plot numerical/datetime values on a continuous axis to show any gaps
        in values. The default is True.
    filename : string
        None or a filename for the figure
    overwrite : boolean
        False (do not overwrite file; the default) or True (overwrite file if
        it exists)
    fig_kw : dictionary
    ...
    **kwargs : dictionary
    ...
    None.
    """
    if tasktype in ['datetime distribution', 'scalars', 'value counts']:
        axkwargs = ax_kw.copy()
        ...
        for l1 in range(nrows):
            for l2 in range(ncols):
                ...
                if isinstance(data, pd.Series):
                    ...
                    plotdata = data[data.columns[cnum]
                                    ] if cnum < data_ncol else None

                # plotdata = data if isinstance(data, pd.Series) else
                # data[data.columns[cnum]] if cnum < data_ncol else None

                if plotdata is not None:
                    # If a list of titles is provided then apply the title for
                    # this plot
                    kw = 'title'

                    if kw in ax_kw and isinstance(ax_kw[kw], list):
                        # Need to check whether title is a list, so the
                        # suptitle() isn't used
                    ...
                    # This check is made because the grid may contain more
                    # cells than there are dataframe columns
                    if tasktype == 'scalars':
                        scalar_bar(plotdata, ax_input=ax, vert=vert,
                                   xlabels_rotate=xlabrot,
                                   perceptual_threshold=perceptual_threshold,
                                   legend=legend, filename=filename,
                                   overwrite=overwrite, ax_kw=axkwargs,
                                   legend_kw={}, **kwargs)

                        cnum += 1
                    elif tasktype == 'datetime distribution':
                        datetime_counts(plotdata, ax_input=ax,
                                        xlabels_rotate=xlabrot,
                                        component=comps[dnum],
                                        gap_threshold=gap_threshold,
                                        show_gaps=show_gaps, filename=filename,
                                        overwrite=overwrite, ax_kw=axkwargs,
                                        **kwargs)
                        ...
                        lollipop(plotdata.value_counts(), ax_input=ax,
                                 vert=vert, xlabels_rotate=xlabrot,
                                 datalabels=datalabels,
                                 continuous_value_axis=continuous_value_axis,
                                 filename=filename, overwrite=overwrite,
                                 ax_kw=axkwargs, **kwargs)
                        cnum += 1
        ...
        print('** WARNING ** vizdataquality, plot.py, plotgrid(): The ' +
              'tasktype is not valid:', tasktype)


def multiplot(plottype, data, perceptual_threshold=0.05,
              number_of_variables_per_row=None, vert=True, xlabels_rotate=0.0,
              datalabels=False, continuous_value_axis=True, filename=None,
              overwrite=False, plt_kw={}, fig_kw={}, ax_kw={}, legend_kw={},
              **kwargs):
    """
    Plot a data quality attribute (e.g., number of missing values in each
    variable).
    The variables can be plotted on multiple rows of bar charts.
    The length of each bar can be adjusted to ensure that important perceptual
    ...
    data : dataframe (violinplot only) or series (all plot types)
        'bar', 'box', 'dot-or-whisker': Series containing the variable names
        (index) and data quality attribute to be plotted (e.g., number of
    ...
    number_of_variables_per_row : int
        None (plot all variables in one bar chart) or the number of variables
        to show in each row
    vert: boolean
        True (vertical bars; the default) or False (horizontal)
    xlabels_rotate : float
    ...
    legend_kw : dictionary
        ...
        Only used if plottype = 'bar'
    kwargs : dictionary
        Keyword arguments for a Matplotlib Axes.bar (scalarbar), Axes.bxp
        (boxplot), Axes.scatter and Axes.errorbar (dot_whisker), Axes.scatter
        and Axes.plot (lollipop) or violinplot object (violin)
    ...
    if isinstance(data, pd.DataFrame) and plottype != 'violin':
        print('** WARNING ** vizdataquality, plot.py, multiplot(): A ' +
              'dataframe cannot be used with a', plottype, 'plot')
    elif plottype in ('bar', 'box', 'dot-or-whisker', 'violin', 'lollipop'):
        axkwargs = ax_kw.copy()
        # Calculate the number of rows/columns of plots
        if plottype == 'violin':
            if isinstance(data, pd.DataFrame):
                num_subplots = int(
                    (data.shape[1] +
                     number_of_variables_per_row - 1) /
                    number_of_variables_per_row)
            else:
                num_subplots = 1
        else:
            ...
            try:
                num_subplots = int(
                    (len(data) + number_of_variables_per_row - 1) /
                    number_of_variables_per_row)
        ...
        if True:
            # This shouldn't be needed because "When subplots have a shared
            # x-axis along a column, only the x tick labels of the bottom
            # subplot are created". Similarly for the Y axis
            ...
            fig.supxlabel(axkwargs.get(
                'xlabel', label1 if vert else default_axis_label))
            axkwargs['xlabel'] = None
            fig.supylabel(axkwargs.get(
                'ylabel', default_axis_label if vert else label1))
        ...
        for l1 in range(num_subplots):
            ...
            if plottype == 'bar':
                ...
                scalar_bar(data.iloc[start:end],
                           perceptual_threshold=perceptual_threshold,
                           number_of_variables_per_row=number_of_variables_per_row,
                           ax_input=ax, vert=vert,
                           xlabels_rotate=xlabels_rotate,
                           datalabels=datalabels, legend=legend,
                           filename=filename, overwrite=overwrite,
                           ax_kw=axkwargs, legend_kw=legend_kw, **kwargs)
            elif plottype == 'box':
                boxplot(data.iloc[start:end],
                        number_of_variables_per_row=number_of_variables_per_row,
                        ax_input=ax, vert=vert, xlabels_rotate=xlabels_rotate,
                        filename=filename, overwrite=overwrite, ax_kw=axkwargs,
                        **kwargs)
            elif plottype == 'dot-or-whisker':
                dot_whisker(data.iloc[start:end],
                            number_of_variables_per_row=number_of_variables_per_row,
                            ax_input=ax,
                            vert=vert, xlabels_rotate=xlabels_rotate,
                            filename=filename, overwrite=overwrite,
                            ax_kw=axkwargs, **kwargs)
            elif plottype == 'lollipop':
                lollipop(plotdata[start:end],
                         number_of_variables_per_row=number_of_variables_per_row,
                         ax_input=ax, vert=vert, xlabels_rotate=xlabels_rotate,
                         datalabels=datalabels,
                         continuous_value_axis=continuous_value_axis,
                ...
                violinplot(data[data.columns[start:end]],
                           number_of_variables_per_row=number_of_variables_per_row,
                           ax_input=ax, vert=vert, xlabels_rotate=xlabels_rotate,
                           filename=filename, overwrite=overwrite,
                           ax_kw=axkwargs, **kwargs)
...
# Functions for summary plots
# =============================================================================
def scalar_bar(data, perceptual_threshold=0.05,
               number_of_variables_per_row=None, ax_input=None, vert=True,
               xlabels_rotate=0.0, datalabels=False, legend=True,
               filename=None, overwrite=False, fig_kw={}, ax_kw={},
               legend_kw={}, **kwargs):
    """
    Create a bar chart showing a data quality attribute (e.g., number of
    missing values in each variable).
    The length of each bar can be adjusted to ensure that important perceptual
    differences are visible.
    ...
    ----------
    data : series
        Series containing the variable names (index) and data quality
        attribute to be plotted (e.g., number of missing values in each
        variable)
    perceptual_threshold : float
        Preceptual discontinuity threshold (0.0 - 1.0) or None
    number_of_variables_per_row : int
    ...
    legend: boolean
        True (add a legend, if a stacked bar chart is plotted) or
        False (no legend)
    filename : string
        None or a filename for the figure
    overwrite : boolean
        False (do not overwrite file; the default)
        or True (overwrite file if it exists)
    fig_kw : dictionary
        Keyword arguments for a Matplotlib Figure object
    ax_kw : dictionary
        Keyword arguments for a Matplotlib Axes object
    ...
    if perceptual_threshold is None:
        ...
        # Apply perceptual discontinuity threshold so that non-zero bars are
        # visible and almost complete variables do not look complete
        plotdata = apply_perceptual_discontinuity_individually(
            data, perceptual_threshold, axis_limits)
        edgecolour = [0, 0, 0]
        stack_colours = [[1, 1, 1], [0.67, 0.67, 0.67],
    ...
    try:
        if isinstance(axis_limits, tuple):
            # Threshold defines whether an axis label will be drawn at the end
            # of a bar or within it
            threshold = (axis_limits[1] - max(0, axis_limits[0])) * 0.2
    except:
        pass

    try:
        # If necessary, extend the arrays to accommodate any dummy variables
    ...
    if stack is None:
        ...
        for key, value in stack.items():
            if value.max() > 0:
                ...
                if vert:
                    p = ax.bar(np.arange(num_bars_in_row), value,
                               bottom=bottom, label=key, ec=edgecolour,
                               color=stack_colours[l1], **kw)
                else:
                    p = ax.barh(np.arange(num_bars_in_row), value, left=bottom,
                                label=key, ec=edgecolour,
                                color=stack_colours[l1], **kw)
                ...
                if datalabels:
                    ...
                    if threshold is None:
                        # Draw axis labels  at the end of the bars
                        bar_labels = ['' if row_values[i] == 0 else str(
                            data.values[i]) for i in range(len(data))]

                        if num_bars_in_row > len(bar_labels):
                            bar_labels += [''] * \
                                (num_bars_in_row - len(bar_labels))

                        ax.bar_label(p, labels=bar_labels,
                                     padding=label_padding)
                    else:
                        # Threshold defines whether an axis label will be
                        # drawn at the end of a bar or within it
                        # NB: There should be only one item in the stack for
                        # each stacked bar
                        bar_labels = ['' if value[i] < threshold else str(
                            data.values[i]) for i in range(len(data))]
                        ...
                        bar_labels = ['' if value[i] >= threshold or value[i] == 0 else str(
                            data.values[i]) for i in range(len(data))]
        ...
        if legend:
            ax.legend(**legend_kw)
        # Add legend to the figure, so there is only one legend if multiplot()
        # was called
        # fig=plt.gcf()
...
def table(data, ax_input=None, include_index=False, auto_column_width=True,
          filename=None, overwrite=False, fig_kw={}, ax_kw={}, **kwargs):
    """
    Plot a table
    ...
    ax_kw : dictionary
        Keyword arguments for a Matplotlib Axes object
    kwargs : dictionary
        Keyword arguments for a Matplotlib Axes.table object. A useful one is
        loc='center'
    ...
    if isinstance(data, pd.Series):
        if include_index:
            ...
            plotdata.rename(
                columns={plotdata.columns[0]: 'Variable'}, inplace=True)
        else:
            plotdata = data
    else:
        # Dataframe
...
def text(plotdata, number_of_variables_per_row=None, ax_input=None,
         legend=True, xlabels_rotate=0.0, filename=None, overwrite=False,
         fig_kw={}, ax_kw={}, **kwargs):
    """
    ...
    filename : string
        None or a filename for the figure
    overwrite : boolean
        False (do not overwrite file; the default) or
...
def boxplot(data, number_of_variables_per_row=None, ax_input=None, vert=True,
            xlabels_rotate=0.0, filename=None, overwrite=False, fig_kw={},
            ax_kw={}, **kwargs):
    """
    ...
    for index, value in data.items():
        ...
        if len(strvals) == 5:
            ...
            if numeric_boxplot:
                # Assume the values are numbers, so convert the values to
                # floats
                vals = list(map(float, strvals))
                # Append this box plot's parameters to the list
                boxes.append({'label': index, 'whislo': vals[0], 'q1': vals[1],
                              'med': vals[2], 'q3': vals[3], 'whishi': vals[4],
                              'fliers': []})
            else:
    ...
    try:
        ...
        time_axis = True if (minmax[1] - minmax[0]) / \
            np.timedelta64(1, 'h') <= 24 else False
    except:
        pass
        time_axis = False
    #
    ...
    if len(boxes) == 0:
        print(
            '** WARNING ** vizdataquality, plot.py, boxplot(): No ' +
...
def violinplot(data, number_of_variables_per_row=None, ax_input=None,
               vert=True, xlabels_rotate=0.0, filename=None, overwrite=False,
               fig_kw={}, ax_kw={}, **kwargs):
    """
...
def dot_whisker(data, number_of_variables_per_row=None, ax_input=None,
                vert=True, xlabels_rotate=0.0, filename=None, overwrite=False,
                fig_kw={}, ax_kw={}, **kwargs):
    """
    Create a dot-or-whisker plot (e.g., to show value lengths for each
    variable)
    ...
    data : series
        Series containing the variable names (index) and data quality
        attribute to be plotted (e.g., number of missing values in
        each variable)
    number_of_variables_per_row : int
        None (plot all variables in one bar chart) or the number of variables
        to show in each row
    ...
    if len(dotx) == 0 and len(whiskerx) == 0:
        print('** WARNING ** vizdataquality, plot.py, ' +
              'dot_whisker(): No variables to be plotted.')
    else:
        # The index contains the variable names
        xlabels = data.index.to_list()

        try:
            # If necessary, extend the arrays to accommodate any dummy
            # variables at the end of the plot
        ...
        if vert:
            ...
            if len(whiskerx) > 0:
                ax.errorbar(whiskerx, whiskery, yerr=whiskererr,
                            fmt='none', capsize=4, **kw)
            ...
            # For multiplot(), this prevents Matplotlib from autoscaling the
            # shared Y axis if 'ylim' not in axkwargs:
            ...
            if len(whiskerx) > 0:
                ax.errorbar(whiskery, whiskerx, xerr=whiskererr,
                            fmt='none', capsize=4, **kw)
            ...
            # For multiplot(), this prevents Matplotlib from autoscaling the
            # shared Y axis if 'xlim' not in axkwargs:
            # By default, start the X axis at the origin axkwargs['xlim'] = 0
...
def lollipop(data, number_of_variables_per_row=None, ax_input=None, vert=True,
             xlabels_rotate=0.0, datalabels=False, continuous_value_axis=True,
             filename=None, overwrite=False, fig_kw={}, ax_kw={}, **kwargs):
    """
    ...
    number_of_variables_per_row : int
        None (plot all variables in one bar chart) or the number of variables
        to show in each row. The default is None.
    ax_input: axis or None
    ...
    if continuous_value_axis:
        ...
        numerical_values = True if all(type(e) in (
            int, float) for e in data.index) else False
    else:
    ...
    if len(dotx) > 0:
        ...
        if vert:
            ...
            if datalabels:
                ...
                for l1 in range(len(dotx)):
                    ax.text(dotx[l1], doty[l1] + offset, str(doty[l1]),
            ...
            if datalabels:
                ...
                for l1 in range(len(dotx)):
                    ax.text(doty[l1] + offset, dotx[l1], str(doty[l1]),
            ...
            # NB: AT one point this was commented out because it causes a
            # crash for some plots if axkwargs contains 'xticks'
            #
            # Now commented out because it sometimes causes too many tick
            # labels
        ...
        print(
            '** WARNING ** vizdataquality, plot.py, lollipop(): ' +
            'No variables to be plotted.')

    if ax_input is None:
        _draw_fig(filename, overwrite)


def datetime_counts(data, component='raw data', gap_threshold=None,
                    show_gaps=True, ax_input=None, xlabels_rotate=0.0,
                    filename=None, overwrite=False, fig_kw={}, ax_kw={},
                    **kwargs):
    """
    Plot the overall distribution of a datetime variable, or the distribution
    of a specific component (e.g., month).
    ...
    component : string
        Component to plot ('year', 'month', 'dayofweek', 'hour', 'minute' or
        'second'; case independent) or 'raw data' (default)
    gap_threshold: None, int or datetime
        None (threshold will be based on the component of the data;
        the default) or value (threshold to use). Only used if component is
        specified.
    show_gaps: boolean
        True (the default) or False (draw lines across gaps). Only used if
    ...
    """
    if len(data) == 0:
        print('** WARNING ** vizdataquality, plot.py, datetime_counts(): ' +
              'There is no data to be plotted.')
    elif component.lower() in ['raw data', 'year', 'month', 'dayofweek',
                               'hour', 'minute', 'second']:
        axkwargs = ax_kw.copy()
        kw = kwargs.copy()
        ...
        # Calculate the number of times each value occurs, sort them and store
        # the result in a data frame
        # value_counts = data.value_counts().to_frame().reset_index().rename(
            # columns={'index': 'Value', data.name: 'Count'})
        ...
        # The dataframe's first column contains the values and the second
        # column contains the count
        value_counts.rename(
            columns={cols[0]: 'Value', cols[1]: 'Count'}, inplace=True)
        ...
        if comp == 'raw data':
            ...
            num_pixels = plt.rcParams['figure.figsize'][0] * \
            ...
            if len(value_counts) > max_unique:
                ...
                for u in ['Y', 'M', 'D', 'h', 'm', 's']:
                    s = value_counts['Value'].apply(
                        lambda x: np.datetime64(x, u))
                    # if len(value_counts['Value'].apply(lambda x:
                    # np.datetime64(x, u)).unique()) <= max_unique:
                ...
                if granularity is not None:
                    # value_counts['Value2'] = value_counts['Value']
                    # .apply(lambda x: np.datetime64(x, granularity))
                    grouped = value_counts[['Value2', 'Count']].groupby(
                        'Value2', sort=True)['Count'].sum().reset_index()
            ...
            if comp == 'year':
                ...
                if xmax - xmin <= 0.0:
                    # The data is only for a single year,
                    # so make the X axis span 3 years
                    # xticklabels = [xmin]
                    xmin -= 0.5
                    xmax += 0.5
        ...
        if comp == 'raw data' or len(groups) > 0:
            ...
            if comp != 'raw data' and show_gaps:
                ...
                for index, value in groups.iloc[1:].items():
                    ...
                    if index <= x[-1] + gthresh:
                        # This item is part of a continuous sequence of
                        # months, etc.
                        x.append(index)
                        y.append(value)
                    else:
                        if len(x) == 1:
                            ...
                            # Plot a polyline (the last item(s) were in a
                            # sequence)
                            ax.plot(x, y, **kw)
            ...
            if xticklabels is None:
                ...
                if True or abs(xlabels_rotate) > 0.0:
                    # Set to true because Matplotlib otherwise rotates the
                    # labels automatically
                    ax.tick_params('x', labelrotation=xlabels_rotate)

                if comp == 'year':
                    # Now commented out because it sometimes causes too many
                    # tick labels
                    # ax.xaxis.set_major_locator(mpl.ticker.MaxNLocator(integer=True))

                    # If there is a small range of years then Matplotlib's
                    # tick labels may include decimals
                    ...
                    ax.set_xticks(new_ticks, new_labels,
                                  rotation=xlabels_rotate)
                ...
                if xinterval is not None:
                    ax.xaxis.set_major_locator(
                        mpl.ticker.MultipleLocator(xinterval))
            else:
                # ax.set_xticks(range(len(xticklabels)))
                # ax.set_xticklabels(xticklabels, rotation=xlabels_rotate)
                if comp == 'year' and len(xticklabels) == 1:
                    ax.set_xticks(
                        range(xticklabels[0], xticklabels[0]+1), xticklabels,
                        rotation=xlabels_rotate)
                else:
                    # xmin used in range because months start from 1, whereas
                    # days of the week start from 0
                    ax.set_xticks(range(xmin, xmin+len(xticklabels)),
                                  xticklabels, rotation=xlabels_rotate)

            # Now commented out as it sometimes causes too many tick labels
            # ax.yaxis.set_major_locator(mpl.ticker.MaxNLocator(integer=True))
            ax.yaxis.set_major_formatter(
                mpl.ticker.FuncFormatter(lambda x, p: format(int(x), ',')))
        else:
            print(
                '** WARNING ** vizdataquality, plot.py, datetime_counts(): ' +
                'No variables to be plotted.')
    else:
        print('** WARNING ** vizdataquality, plot.py, datetime_counts(): ' +
